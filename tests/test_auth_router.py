...
# --- Success case ---
  def test_login_success(self, client, registered_user):
    response = client.post("/login", json={"username": "thomas", "password": "Foo1234"})
    token = jwt.decode(response.json()["token"], key=os.getenv("ACCESS_TOKEN_KEY", "very secret key"), algorithms=["HS256"])
  ...
  def test_login_wrong_password(self, client, registered_user):
      response = client.post("/login", json={"username": "thomas", "password": "Foo123"})
  ...
  def test_login_wrong_username(self, client, registered_user):
      response = client.post("/login", json={"username": "thomass", "password": "Foo1234"})
  ...
  def test_login_nonexistent_user(self, client):
      response = client.post("/login", json={"username": "ghost", "password": "Foo1234"})
