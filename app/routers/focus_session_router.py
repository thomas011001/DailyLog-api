...
@focus_session_router.post(
    "/focus_steps/{step_id}/sessions", status_code=201, response_model=FocusSessionOut
)
def create_focus_session(step_id: int,current_user: dict = Depends(get_current_user) ,service: FocusSessionService = Depends(get_session_sevice)):
    try:
